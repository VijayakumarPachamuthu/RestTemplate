...
public class MyRestTempl {
	...
	public List<Integer> getPrime81(@PathVariable int a1, @PathVariable int b2) {
		...
		ResponseEntity<List<Integer>> a = rt.exchange(url + a1 + "/" + b2, HttpMethod.GET, null,
	...
	public List<EmployeePojo> getListObject81() {
		...
		ResponseEntity<List<EmployeePojo>> a = rt.exchange(url, HttpMethod.GET, null,
				new ParameterizedTypeReference<List<EmployeePojo>>() {
				}); // new ParameterizedTypeReference() used to get the list of object
	...
	public List<EmployeePojo> getBetween81(@PathVariable int s, @PathVariable int e) {
		...
		ResponseEntity<List<EmployeePojo>> a = rt.exchange(url + s + "/" + e, HttpMethod.GET, null,
				new ParameterizedTypeReference<List<EmployeePojo>>() {
				});
		List<EmployeePojo> b = a.getBody();
		return b;
	...
	@GetMapping(value = "/getStartLetter81/{n}")
	public List<EmployeePojo> getStartLetter81(@PathVariable char n) {
		String url = "http://localhost:8082/getStartLetter/";
